...
**A known limit of the severity join, stated because it is a wrong rating and not a blank.** The rating a citation gets is reconstructed from the model's prose, and the module refuses wherever the reading is ambiguous — but one shape gets past the refusal. Where the answer is written as a _shifted_ list (each marker at the end of its line, the name it belongs to on the next) **and** the final citation's forward window is emptied — by a chart citation standing before the dangling name, or by a full stop after the last marker — the answer resolves as a rotation and every rating is the neighbouring finding's. Measured on the demo chart, `Hydrocortisone Injection vial 100mg matters less than [350] / Solu-Medrol 125mg/5ml [352] / [14] Prednisone Co 5mg` renders a Major finding as Moderate and a Moderate one as Major. The obvious fix — widening the forward window past a citation outside the measurement, which is what the backward window already does — was tried and reverted: it closes this shape and costs three correct ratings on a live answer that interleaves two drug families in one line. `src/utils/safety-disclosure.ts` carries the measurements beside the gate.
