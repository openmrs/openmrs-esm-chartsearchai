...
The fourth is a residual of the rule that reads a hyphen joining two drug names. It examines the text after the hyphen only as far as the next space, so it cannot see a sibling drug whose name contains one, and it does not fire at all unless the name it matched is itself next to the hyphen — so `Solu-Medrol 125mg/5ml-Prednisone` renders one drug's rating for a sentence naming two. Left open because every narrower rule tried fails one of the cases already committed, and the resolver records which and why; a third boundary rule written in a hurry is how the first two of these classes were created.
